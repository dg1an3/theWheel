...
BEGIN_MESSAGE_MAP(CSpaceClassDlg, CDialog)
	...
	ON_WM_SIZE()
	ON_WM_DRAWITEM()
	ON_NOTIFY(LVN_ITEMACTIVATE, IDC_CLASSLIST, OnItemactivatedClasslist)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
{
	int nIndex = 0;
	POSITION pos = m_mapClassColors.GetStartPosition();
	while (NULL != pos)
	{
		CString strClass;
		COLORREF color;
		m_mapClassColors.GetNextAssoc(pos, strClass, (void*&) color);
...
BOOL CSpaceClassDlg::OnInitDialog() 
{
    ...
    m_ClassList.InsertColumn(0, "Class", LVCFMT_LEFT, rect.Width() / 2, 0); 
    m_ClassList.InsertColumn(1, "Color", LVCFMT_LEFT, rect.Width() / 2, 1); 
...
void CSpaceClassDlg::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	dc.ExtTextOut(rectClass.left, rectClass.top, ETO_CLIPPED, rectClass,
		strClass, NULL);
...
void CSpaceClassDlg::OnItemactivatedClasslist(NMHDR* pNMHDR, LRESULT* pResult) 
{
	NM_LISTVIEW* pNMListView = (NM_LISTVIEW*)pNMHDR;
	
	// get the currently activated class
