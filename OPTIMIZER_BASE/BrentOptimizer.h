...
public:
	...
	//  tol is variable specifing the minimum agreement needed between successive intertions before stopping 
...
template<class TYPE>
class CGradBrentOptimizer : public COptimizer<1, TYPE>
{
public:
				...
				iteration.Set(iteration.Get()+1))
		{
			...
			if (fabs(e) > tol1)
			{
				...
				if (dw != dx) 
					...
					d1 = 2.0 * (b - a);	// initialize with out-of-bracket value

				TYPE d2;	
				if (dv != dx) 
					d2 = (v - x) * dx / (dx - dv);
				else
					d2 = 2.0 * (b - a);	// initialize with out-of-bracket value
