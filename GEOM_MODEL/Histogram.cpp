...
void CHistogram::SetVolume(CVolume<double> *pVolume)
{
	if (NULL != m_pVolume)
	{
		m_pVolume->GetChangeEvent().RemoveObserver(this, 
			(ListenerFunction) OnVolumeChange);
	}
	...
	if (NULL != m_pVolume)
	{
		m_pVolume->GetChangeEvent().AddObserver(this, 
			(ListenerFunction) OnVolumeChange);
	}
...
CArray<double, double>& CHistogram::GetBins()
{
	if (m_bRecomputeBins)
	{
		...
		double max = m_pVolume->GetMax();
		double min = m_pVolume->GetMin();
		...
		for (int nAtZ = 0; nAtZ < m_pVolume->GetDepth(); nAtZ++)
		{
			for (int nAtY = 0; nAtY < m_pVolume->GetHeight(); nAtY++)
			{
				for (int nAtX = 0; nAtX < m_pVolume->GetWidth(); nAtX++)
				{
					...
					// compute the bin
					m_arrBins[(int)((intensity - min) / range * 255.0)] += 
