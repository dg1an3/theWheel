...
void CObservableEvent::AddObserver(CObject *pObserver, ListenerFunction func) const
{
...
void CObservableEvent::RemoveObserver(CObject *pObserver, ListenerFunction func) const
{
	for (int nAt = m_arrObservers.GetSize()-1; nAt >= 0; nAt--)
	{
		if (m_arrObservers[nAt] == pObserver && m_arrFunctions[nAt] == func)
