...
// Initialize a subdivision to the triangle defined by the points a, b, c.
{
	...
	Edge* ea = MakeEdge();
	ea->EndPoints(da, db);
	m_arrQuadEdges.Add(ea->Qedge());

	Edge* eb = MakeEdge();
	Splice(ea->Sym(), eb);
	eb->EndPoints(new Point2d(*db), dc);
	m_arrQuadEdges.Add(eb->Qedge());

	Edge* ec = MakeEdge();
	Splice(eb->Sym(), ec);
	ec->EndPoints(new Point2d(*dc), new Point2d(*da));
	Splice(ec->Sym(), ea);
	m_arrQuadEdges.Add(ec->Qedge());
...
// Additionally, the data pointers of the new edge are set.
{
	...
	Splice(e, a->Lnext());
	Splice(e->Sym(), b);
	e->EndPoints(new Point2d(*a->Dest()), new Point2d(*b->Org()));
	return e;
}
...
// modifications and a bug fix.
{
	...
	// existing edge.)
	Edge* base = MakeEdge();
	m_arrQuadEdges.Add(base->Qedge());

	base->EndPoints(new Point2d(*e->Org()), new Point2d(x));
	Splice(base, e);
	startingEdge = base;
	do {
...
// function pointer to a supplied function for drawing lines
extern void DrawLine(double *a, double *b);
