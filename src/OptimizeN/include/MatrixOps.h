	...
	// orthogonalizes the input matrix using GSO
{
	...
	for (int nAtCol = 1; nAtCol < mMat.GetCols(); nAtCol++)
	{
				...
				nAtOrthoCol--)
		{
			...
			typename MATRIX_TYPE::ELEM_TYPE scalar = (mMat[nAtCol] * mMat[nAtOrthoCol]) 
	...
	// invert the matrix
{
	...
	typename MATRIX_TYPE::COL_TYPE vRow(mMat.GetCols());
	typename MATRIX_TYPE::COL_TYPE vOtherRow(mMat.GetCols());
	...
	for (int nCol = 0; nCol < mMat.GetCols(); nCol++)
	{
		...
		typename MATRIX_TYPE::ELEM_TYPE scale = R(1.0) / mCopy[nCol][nCol];	
	...
	// swaps two rows of the matrix
{
	...
	if (nRow1 != nRow2)
	{
		...
		{
			typename MATRIX_TYPE::ELEM_TYPE temp = mMat[nAtCol][nRow1];
	...
	// swaps two COLUMNS of the matrix
{
	...
	if (nCol1 != nCol2)
	{
		...
		typename MATRIX_TYPE::COL_TYPE vTemp = mMat[nCol1];	
