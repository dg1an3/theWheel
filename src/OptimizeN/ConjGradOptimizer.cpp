	...
	for (m_nIteration = 0; (m_nIteration < ITER_MAX) && !bConvergence; m_nIteration++)
	{
		...
		if (m_bCalcVar)
		{
			...
			for (int nDim = 0; nDim < m_vDir.GetDim(); nDim++)
			{
				...
				m_vAdaptVariance[nDim] = (REAL)1.0 / (projSearchedSq_scaled + projNSSq / m_varMax);
